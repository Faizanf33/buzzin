...
def get_all_users(admin_id: int):
    """Get all users"""
    response_object = {"status": False, "message": "Invalid payload."}
...
@authenticate
def get_single_user(admin_id: int, user_id: int):
    """Get single user"""
    response_object = {"status": False, "message": "Invalid payload."}
...
def find_user(user_id: int):
    ...
    try:
        ...
        if email:
            ...
            user = user.filter(User.username.like(f"%{username}%"))

        elif firstname:
            user = user.filter(User.firstname.like(f"%{firstname}%"))

        elif lastname:
            user = user.filter(User.lastname.like(f"%{lastname}%"))

        else:
            response_object["message"] = "No matching user found."
...
def user_settings(user_id: int):
    ...
    try:
        ...
        if topics:
            ...
            topics = [topic.lower() for topic in topics if topic.lower() in TOPICS]
